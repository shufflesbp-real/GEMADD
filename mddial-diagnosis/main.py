...
def main():
    ...
    for k, v in dict_patient_test.items():
        try:
            ...
            # if use_rank_aggregation and predicted_list:
            #     final_ppr_scores = calc_dis_rank_1(new_graph, dialogue_id)
            #     final_ppr_scores = {d: s for d, s in final_ppr_scores.items() if d in possible_set_of_diseases}
            #     confirmed_symptoms = []
            #     for u, n, data in new_graph.edges(data=True):
            #         if u == dialogue_node and new_graph.nodes[n].get('type') == 'symptom':
            #             if data.get('weight', 0) > 0:
            #                 confirmed_symptoms.append(n)
            #     co_occ_scores = compute_cooccurrence_scores(
            #         current_diseases=list(final_ppr_scores.keys()), 
            #         patient_symptoms=confirmed_symptoms,
            #         p_d_given_s=p_d_given_s,
            #         method='avg' 
            #     )
                
            #     aggregated_scores = reciprocal_rank_fusion(final_ppr_scores, co_occ_scores)
            #     predicted_list = list(aggregated_scores.keys())
            #     predicted = predicted_list[0] if predicted_list else "None"
            # =================================================================

            # METRIC CALCULATIONS
            x1 = set(s.lower() for s in disease_symptom_dict.get(ground_truth, []))
    ...
    result_df.to_csv('/home/Byomakesh/Unified-medical-KG/mddial-diagnosis_new/results-other.csv', index=False)
    print("Results saved to: /home/Byomakesh/Unified-medical-KG/mddial-diagnosis_new/results-other.csv")
    calculate_and_print_all_metrics(result_df,elapsed_time=elapsed_time)
    ...
    for k, v in dict_patient_test.items():
        try:
            ...
            if should_stop_conversation(possible_set_of_diseases, output, 0):
                ...
                # use_rank_aggregation = True
                # if use_rank_aggregation and predicted_list:
                #     # 1. Get raw PPR scores from the final graph
                #     # We re-run this to get the dictionary {disease: score} for the current graph state
                #     final_ppr_scores = calc_dis_rank_1(new_graph, dialogue_id)
                    
                #     # Filter ppr scores to only include diseases currently in our candidate list
                #     # This respects the pruning that happened during the dialogue
                #     final_ppr_scores = {d: s for d, s in final_ppr_scores.items() if d in possible_set_of_diseases}
                    
                #     # 2. Extract confirmed positive symptoms from the graph
                #     # We look for edges from User -> Symptom with weight > 0
                #     confirmed_symptoms = []
                #     for u, n, data in new_graph.edges(data=True):
                #         if u == dialogue_node and new_graph.nodes[n].get('type') == 'symptom':
                #             if data.get('weight', 0) > 0:
                ...
                #     # 3. Compute Co-occurrence Scores
                #     co_occ_scores = compute_cooccurrence_scores(
                #         current_diseases=list(final_ppr_scores.keys()), 
                #         patient_symptoms=confirmed_symptoms,
                #         p_d_given_s=p_d_given_s,
                ...
                #     # 5. OVERWRITE the predictions with the new Aggregated List
                #     predicted_list = list(aggregated_scores.keys())
                #     predicted = predicted_list[0] if predicted_list else "None"
